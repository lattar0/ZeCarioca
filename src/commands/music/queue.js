...
module.exports = class QueueCommand extends Command {
  ...
  async run ({ message, author, channel, member }) {
    ...
    message.channel.sendTimeout(queueEmbed.setTitle(`Tocando agora: \`${player.track.title}\``).setDescription(`${serverQueue.map(a => `\`${++index}.\` **${a.title}** - *${a.requester.username}#${a.requester.discriminator}*`).join('\n')}`))
